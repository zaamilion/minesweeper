...
def gen_desk(width, height, quantity_of_bombs, first_step):
    ...
    desk = Desk(height=height, width=width, quantity_of_bombs=quantity_of_bombs, slots=slots)
    bombs_xy = set([random.randint(0, height*width - 1) for _ in range(quantity_of_bombs)])
